...
if args.trends:
    ...
    EnvConfig.read(ENVCONFIG)
    mngdb = mongodata()
    user_id_name = mngdb.get_users()
    users={}
    #authenticate("{0}:{1}".format(EnvConfig.get('neo4j', 'ip'),EnvConfig.get('neo4j', 'port')), EnvConfig.get('neo4j', 'user'), EnvConfig.get('neo4j', 'password'))

    stringconnect="http://{0}:{1}@{2}:{3}/db/data/".format(EnvConfig.get('neo4j', 'user'),
                                               EnvConfig.get('neo4j', 'password'),
                                               EnvConfig.get('neo4j', 'ip'),
                                               EnvConfig.get('neo4j', 'data'))
    print stringconnect
    g = Graph(stringconnect)
    tx = g.begin()
    for x in user_id_name:
        users[x.values()[0]]=x.values()[1]
    relation = mngdb.get_relationship('followers')
    for i in relation:
        a = Node("User", screen_name=users[i['friend']], id=i['friend'])
        print ".",
        tx.create(a)
        b = Node("User", screen_name=users[i['user']], id=i['user'])
        print ".",
        tx.create(b)
        ...
        tx.create(ab)
        tx.commit()
        #print ("({0})->({1})").format(users[i['friend']], users[i['user']])
    relation = mngdb.get_relationship('friends')
    for i in relation:
        a = Node("User", screen_name=users[i['user']], id=i['friend'])
        print ".",
        tx.create(a)
        b = Node("User", screen_name=users[i['friend']], id=i['user'])
        print ".",
        tx.create(b)
        ab = Relationship(a, "FOLLOW", b)
        print ".",
        tx.create(ab)
        tx.commit()
        # print ("({0})->({1})").format(users[i['user']], users[i['friend']])
    g.close()
    #print (json.dumps(myself, indent=1))
    # http://py2neo.org/v4/database.html#py2neo.database.Transaction.create